...
后续 Codex 请求使用所选活动账户，每个对话不会各自绑定账户。每个已保存账户也会作为独立的模型选择路由提供，且该路由在认证和 token 刷新期间始终使用自己的账户。Fast Mode 才是会话级设置。取消新的授权会保留已有账户；明确的刷新凭据撤销响应会提示重新授权，临时故障则保留账户供重试。详见[账户行为](reference.zh.md#账户模型与额度)。
