...
The picker overlays a live Codex model catalog on the installed `@earendil-works/pi-ai` records. The plugin supplies `gpt-6-astra` when pi-ai lacks it, and adds `gpt-6-sol` and `gpt-6-luna` only when the live catalog or official CLI cache advertises those slugs. Each new variant borrows the corresponding GPT-5.6 variant's tool and reasoning-level mapping; its name and context limits come from the official catalog. Configure `models` to include their ids if you use an allowlist. The catalog is not proof of inference access; account entitlement, quota and network behavior remain OpenAI-controlled.
