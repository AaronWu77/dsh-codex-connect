...
*/
function openAICodexAccountRouteLabelFor(account, duplicateNames) {
	const name = account.displayName.trim();
	if (name.length === 0) return openAICodexAccountRouteLabel(account.accountKey);
	return duplicateNames.has(name) ? `${OPENAI_CODEX_PRIMARY_DISPLAY_NAME} (${name} \u00b7 ${openAICodexAccountKeySuffix(account.accountKey)})` : `${OPENAI_CODEX_PRIMARY_DISPLAY_NAME} (${name})`;
}
	...
	*/
	reconcile(accounts) {
		...
		const routes = [{
			...
			displayName: active === void 0 ? OPENAI_CODEX_PRIMARY_DISPLAY_NAME : openAICodexAccountRouteLabelFor(active, duplicateNames)
		}];
		for (const account of accounts) {
			...
			routes.push({
				routeId,
				displayName: openAICodexAccountRouteLabelFor(account, duplicateNames),
...
function parseResetCreditTimestamp(value, field) {
	if (value === void 0 || value === null) return void 0;
	if (typeof value === "number") {
		if (!Number.isSafeInteger(value) || value < 0) throw new Error(`OpenAI Codex returned an invalid reset-credit ${field}`);
		return value;
	}
	if (typeof value === "string" && RESET_CREDIT_TIMESTAMP.test(value)) {
		const milliseconds = Date.parse(value);
		if (Number.isFinite(milliseconds) && milliseconds >= 0) return Math.floor(milliseconds / 1e3);
	}
	throw new Error(`OpenAI Codex returned an invalid reset-credit ${field}`);
}
function parseResetCredit(value) {
	if (!isRecord$3(value)) throw new Error("OpenAI Codex returned a malformed reset credit");
	const id = value["id"];
	if (typeof id !== "string" || id.length === 0 || id.length > 256) throw new Error("OpenAI Codex returned an invalid reset-credit id");
	const resetType = value["reset_type"];
	if (typeof resetType !== "string" || resetType.length === 0 || resetType.length > 128) throw new Error("OpenAI Codex returned an invalid reset-credit type");
	const status = value["status"];
	if (typeof status !== "string" || status.length === 0 || status.length > 128) throw new Error("OpenAI Codex returned an invalid reset-credit status");
	const grantedAt = parseResetCreditTimestamp(value["granted_at"], "grant time");
	if (grantedAt === void 0) throw new Error("OpenAI Codex returned a reset credit without a grant time");
	const expiresAt = parseResetCreditTimestamp(value["expires_at"], "expiry time");
	const title = value["title"];
	if (title !== void 0 && title !== null && (typeof title !== "string" || title.length === 0 || title.length > 256)) throw new Error("OpenAI Codex returned an invalid reset-credit title");
	return {
		id,
		resetType,
		status,
...
function parseResetCredits(value) {
	if (value === void 0 || value === null) return void 0;
	if (!isRecord$3(value)) throw new Error("OpenAI Codex returned malformed reset-credit details");
	const availableCount = value["available_count"];
	if (typeof availableCount !== "number" || !Number.isSafeInteger(availableCount) || availableCount < 0) throw new Error("OpenAI Codex returned an invalid reset-credit count");
	const entries = value["credits"];
	if (entries !== void 0 && entries !== null && !Array.isArray(entries)) throw new Error("OpenAI Codex returned malformed reset-credit entries");
	const credits = entries === void 0 || entries === null ? void 0 : entries.map(parseResetCredit);
	return {
		availableCount,
...
*/
async function readResetCreditDetails(access, accountId, signal) {
	...
	if (parsed === void 0) throw new Error("OpenAI Codex returned no reset-credit details");
	return parsed;
}
...
async function readOpenAICodexRateLimits(store) {
	...
	if (summary === void 0 || summary.credits !== void 0 || summary.availableCount === 0) return usage;
	try {
		const details = await readResetCreditDetails(access, accountId, signal);
		return details.credits === void 0 ? usage : {
			...usage,
			resetCredits: {
				...summary,
...
//#endregion
export { OPENAI_CODEX_MODEL_CATALOG_TTL_MS as $, OPENAI_CODEX_AUTH_FILENAME as $t, VIEW_IMAGE_TOOL_NAME as A, resolveOpenAICodexSettings as At, PI_AI_PACKAGE as B, readOpenAICodexRateLimits as Bt, OPENAI_CODEX_IMAGE_REQUEST_TIMEOUT_MS as C, isValidOpenAICodexContextWindowOverrides as Ct, OpenAICodexTransport as D, isValidOpenAICodexProxyUrl as Dt, OPENAI_CODEX_TRANSPORT_SERVICE as E, isValidOpenAICodexModelCatalogClientVersion as Et, CODEX_CONNECT_VERSION as F, isFastModeSessionId as Ft, SUPPORTED_PI_AI_RANGE as G, decodeOpenAICodexModelCatalogStatus as Gt, SUPPORTED_DSH_PLUGIN_API_VERSION as H, OPENAI_CODEX_MODEL_CATALOG_PATH as Ht, COMPATIBILITY_CONTRACT as I, OpenAICodexTrustedOriginsStore as It, evaluateCompatibility as J, loginOpenAICodex as Jt, assessCompatibility as K, openAICodexContextLimit as Kt, COMPATIBILITY_PACKAGES as L, normalizeTrustedOrigin as Lt, assertNoOpenAICodexProviderConflict as M, FastModeRegistry as Mt, diagnoseOpenAICodex as N, OPENAI_CODEX_FAST_MODE_MAX_SESSIONS as Nt, OpenAICodexTransportError as O, normalizeOpenAICodexProxyUrl as Ot, openAICodexConflictMessage as P, OPENAI_CODEX_FAST_MODE_MAX_SESSION_ID_LENGTH as Pt, OPENAI_CODEX_MODEL_CATALOG_CACHE_FILENAME as Q, OPENAI_CODEX_AUTH_DOCUMENT_LIMIT as Qt, COMPATIBILITY_SCHEMA_VERSION as R, OPENAI_CODEX_USAGE_URL as Rt, OPENAI_CODEX_IMAGE_PROMPT_MAX_LENGTH as S, decodeOpenAICodexSettings as St, OPENAI_CODEX_TRANSPORT_ERROR_CODES as T, isValidOpenAICodexMaxTokensOverrides as Tt, SUPPORTED_DSH_PLUGIN_API_VERSIONS as U, OPENAI_CODEX_MODEL_CATALOG_REFRESH_PATH as Ut, SUPPORTED_DSH_PLUGIN_API_RANGE as V, openAICodexModelCatalog as Vt, SUPPORTED_NODE_RANGE as W, OPENAI_CODEX_MODEL_CATALOG_STATUS_PATH as Wt, readInstalledPackageVersion as X, openAICodexAuthStatus as Xt, isSupportedDshPluginApiVersion as Y, logoutOpenAICodex as Yt, OPENAI_CODEX_MODELS_URL as Z, publicAuthError as Zt, mapOpenAICodexSearchResponse as _, DEFAULT_OPENAI_CODEX_SEARCH_MAX_OUTPUT_TOKENS as _t, name as a, OPENAI_CODEX_LOCAL_PROXY_CANDIDATES as at, OPENAI_CODEX_IMAGE_MAX_ERROR_BYTES as b, DEFAULT_OPENAI_CODEX_SETTINGS as bt, migrateOpenAICodexSearchHistory as c, OPENAI_CODEX_PROXY_PROBE_URL as ct, CODEX_AUTO_REVIEW_MODEL as d, listOpenAICodexProxyCandidates as dt, OPENAI_CODEX_AUTH_V1_BACKUP_SUFFIX as en, OpenAICodexModelCatalog as et, probeCodexAutoReview as f, DEFAULT_OPENAI_CODEX_CONTEXT_WINDOW_MODE as ft, OpenAICodexSearchProvider as g, DEFAULT_OPENAI_CODEX_SEARCH_CONTEXT_SIZE as gt, OPENAI_CODEX_SEARCH_URL as h, DEFAULT_OPENAI_CODEX_PROXY_URL as ht, inject as i, OPENAI_CODEX_ACCOUNT_LIMIT as in, OPENAI_CODEX_PROXY_TEST_PATH as it, IMAGE_GENERATE_TOOL_NAME as j, OPENAI_CODEX_FAST_MODE_PATH as jt, isOpenAICodexTransportError as k, resolveOpenAICodexProxyUrl as kt, compareOpenAICodexVersions as l, OpenAICodexProxyManager as lt, OPENAI_CODEX_SEARCH_PROVIDER as m, DEFAULT_OPENAI_CODEX_MODEL_CATALOG_CLIENT_VERSION as mt, OPENAI_CODEX_SETTINGS_NS as n, OpenAICodexCredentialStore as nn, parseOpenAICodexModelsPayload as nt, OPENAI_CODEX_HISTORY_BACKUP_SUFFIX as o, OPENAI_CODEX_PROXY_CANDIDATE_LIMIT as ot, OPENAI_CODEX_BASE_URL as p, DEFAULT_OPENAI_CODEX_IMAGE_MODEL_HINT as pt, detectCompatibility as q, openAICodexModeContextWindow as qt, apply as r, openAICodexAuthPath as rn, OPENAI_CODEX_PROXY_DETECT_PATH as rt, OPENAI_CODEX_SEARCH_MODEL_REQUEST_EVENT as s, OPENAI_CODEX_PROXY_PROBE_TIMEOUT_MS as st, Config as t, OPENAI_CODEX_PROVIDER as tn, openAICodexCliModelCachePath as tt, parseOpenAICodexVersion as u, detectOpenAICodexProxies as ut, OPENAI_CODEX_IMAGE_GENERATION_URL as v, DEFAULT_OPENAI_CODEX_SEARCH_MODE as vt, OPENAI_CODEX_TRANSPORT_API_VERSION as w, isValidOpenAICodexImageModelHint as wt, OPENAI_CODEX_IMAGE_MAX_RESPONSE_BYTES as x, OPENAI_CODEX_SETTINGS_NAMESPACE as xt, OPENAI_CODEX_IMAGE_MAX_COUNT as y, DEFAULT_OPENAI_CODEX_SEARCH_MODEL as yt, DSH_PLUGIN_API_PACKAGES as z, parseOpenAICodexUsage as zt };
