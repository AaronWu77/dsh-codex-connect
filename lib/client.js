window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		*/
		...
		function quotaOf$1(usage, model, windowSeconds) {
		...
		function OpenAICodexQuotaIndicator({ directory, t }) {
			...
			const fiveHour = quotaOf$1(request.usage, model, FIVE_HOUR_SECONDS);
			const weekly = quotaOf$1(request.usage, model, WEEK_SECONDS);
		...
		function createCodexQuotaService(account) {
			...
			const armTimer = () => {
				...
				staleTimer = setTimeout(() => {
					staleTimer = void 0;
					if (current === null) return;
					if (Date.now() - currentAt < STALE_HOLD_MS) {
						armTimer();
						return;
					}
					current = null;
			...
			return {
				...
				subscribe: (listener) => {
					...
					if (unsubscribeAccount === void 0) unsubscribeAccount = account.subscribe(sync);
					sync();
					return () => {
						listeners.delete(listener);
						if (listeners.size > 0 || unsubscribeAccount === void 0) return;
						unsubscribeAccount();
