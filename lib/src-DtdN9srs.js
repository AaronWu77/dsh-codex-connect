...
function withOpenAICodexFastMode(provider, fastMode) {
	...
	return {
		...
		streamSimple(model, context, options) {
			...
			if (!(provider.id === model.provider && isOpenAICodexRouteId(provider.id) && fastMode !== void 0 && fastMode.isEnabled(sessionId))) return streamSimple.call(provider, model, context, options);
...
function createOpenAICodexProfile(provider, fastMode, proxyManager, resolveProxyUrl, contextWindowOverrides, maxTokensOverrides, route = OPENAI_CODEX_PRIMARY_ROUTE, layer) {
...
*/
function createOpenAICodexAdapter(credentials, resolveAttachments, fastMode, visibleModelIds, proxyManager, resolveProxyUrl, contextWindowOverrides, maxTokensOverrides, routeBindings, catalogLayer) {
	...
	const currentProfiles = () => {
		...
		if (profiles === void 0 || !deepEqualJson(previousKey, key)) {
			previousKey = key;
			profiles = new Map(bindings.map((route) => [route.routeId, createOpenAICodexProfile(provider, fastMode, proxyManager, resolveProxyUrl, windowOverrides, tokenOverrides, route, layer)]));
		}
		return profiles;
	};
